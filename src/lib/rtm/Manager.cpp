...
namespace RTC
{
  ...
  bool Manager::activateManager()
  {
    ...
    m_config["sdo.service.consumer.available_services"] = coil::eraseBlank(coil::flatten(SdoServiceConsumerFactory::instance().getIdentifiers()));
  ...
  void Manager::initManager(int argc, char **argv)
  {
    ...
    if (coil::toBool(m_config["manager.shutdown_auto"], "YES", "NO", true) && !coil::toBool(m_config["manager.is_master"], "YES", "NO", false) && m_needsTimer)
    {
      std::chrono::milliseconds tm;
      if ((m_config.findNode("manager.auto_shutdown_duration") == nullptr) || !coil::stringTo(tm,
    ...
    {
      addTask([this]
              { cleanupComponents(); }, std::chrono::seconds(1));
    }
  ...
  void Manager::mainThread()
  {
    ...
    m_threadOrb = std::thread([this]
                              { m_pORB->run(); });

    std::chrono::microseconds period;
    if (m_config["timer.tick"].empty() || !coil::stringTo(period, m_config["timer.tick"].c_str()))
    {
      period = std::chrono::milliseconds(100);
    }

    std::chrono::milliseconds delay;
    if ((m_config.findNode("manager.termination_waittime") == nullptr) || !coil::stringTo(delay, m_config["manager.termination_waittime"].c_str()))
    {
  ...
  bool Manager::initLogger()
  {
    ...
    coil::toBool(m_config["logger.stream_lock"], "enable", "disable", false) ? RTC::Logger::enableLock() : RTC::Logger::disableLock();
  ...
  bool Manager::initNaming()
  {
    ...
    if (coil::toBool(m_config["naming.update.enable"], "YES", "NO", true) && m_needsTimer)
    {
      std::chrono::milliseconds tm;
      if (m_config["naming.update.interval"].empty() || !coil::stringTo(tm, m_config["naming.update.interval"].c_str()))
      {
        tm = std::chrono::seconds(10); // default interval for safety
      }
      addTask([this]
              { m_namingManager->update(); }, tm);
    }
    return true;
  }
  ...
  void Manager::initCpuAffinity()
  {
    ...
    if (m_config.findNode("manager.cpu_affinity") == nullptr)
    {
      return;
    }
    ...
    if (!cpu_list.empty())
    {
      ...
      if (result && !ret_cpu.empty() && !cpu_list.empty() && ret_cpu.size() == cpu_list.size() && std::equal(ret_cpu.begin(), ret_cpu.end(), cpu_list.begin()))
      {
      }
      else
      {
  ...
  bool Manager::initManagerServant()
  {
    ...
    if (coil::toBool(m_config["corba.update_master_manager.enable"], "YES", "NO", true) && !coil::toBool(m_config["manager.is_master"], "YES", "NO", false) && m_needsTimer)
    {
      std::chrono::milliseconds duration;
      if ((m_config.findNode("corba.update_master_manager.interval") == nullptr) || !coil::stringTo(duration, m_config["corba.update_master_manager.interval"].c_str()) || (duration <= std::chrono::seconds::zero()))
      {
        duration = std::chrono::seconds(10);
      }
      addTask([this]
              { m_mgrservant->updateMasterManager(); }, duration);
    }
                                   ...
                                   const coil::Properties &prop)
  {
    ...
    type_prop << name_prop;
    type_prop["config_file"] = coil::flatten(coil::unique_sv(std::move(config_fname)));
      ...
      else
      {
        if (count > 0 && ((count % 2) != 0))
        {
          ...
          else if (c == 'N')
          {
            ...
            str += m_config["os.hostname"];
          else if (c == 'M')
            str += m_config["manager.name"];
          else if (c == 'p')
            str += m_config["manager.pid"];
          else
          {
            str.push_back(c);
          }
  ...
  void Manager::setEndpointProperty(CORBA::Object_ptr objref)
  {
    ...
    for (auto const &endpoint : CORBA_IORUtil::getEndpoints(ior))
    {
      ...
      if (ipv4 && coil::isIPv4(addr))
      {
        ...
        if (ipv4_list.empty() ||
            std::find(ipv4_list.begin(), ipv4_list.end(), ipv4_count) != ipv4_list.end())
        {
      ...
      if (ipv6 && coil::isIPv6(addr))
      {
        std::string tmp("[" + addr + "]:" + coil::otos(endpoint.port));
        if (ipv6_list.empty() ||
            std::find(ipv6_list.begin(), ipv6_list.end(), ipv6_count) != ipv6_list.end())
        {
          epstr.emplace_back(tmp);
                                       ...
                                       bool &ip, std::vector<int> &ip_list)
  {
    ...
    ip = true;
    pos += ipver.size();
    if (pos >= ep_prop.size() || ep_prop[pos] != '(')
    {
      return;
    }
                                                  ...
                                                  const std::string &kind)
  {
    ...
    for (auto const &n : m_namingManager->getNameServices())
    {
      ...
      {
        if (bl[j].binding_type != CosNaming::nobject)
        {
          continue;
        }
        ...
        try
        {
          PortProfile_var p = portsvc->get_port_profile();
        }
        catch (...)
        {
          continue;
        } // the port must be zombie
                                 ...
                                 PortServiceList_var &target_ports)
  {
    ...
    {
      if (port->_is_equivalent(target_ports[i]))
      {
        continue;
      }
